...
class Game(object):
    def __init__(self):
        #Empty board for game
        self.board = [' '] * 9
        
        #Sets current turn to player by default.
        self.current_turn = 'O'

        #On/off switch to restart the game
        self.game_on = False
    ...
    #Print the board for the current match
    def printBoard(self): print(f'Current Board:\n{self.board[6]}|{self.board[7]}|{self.board[8]}\n-+-+-\n{self.board[3]}|{self.board[4]}|{self.board[5]}\n-+-+-\n{self.board[0]}|{self.board[1]}|{self.board[2]}')
    
    #Let the player know how to make moves in the game.
    ...
    #Cycle player/bot moves.
    def runGame(self):
        if self.current_turn == 'O': self.playerMove()
        else: self.botMove()

    #Check if the game is still in progress or not and gets the winner.
    def gameOver(self):
        
        win_check = [self.board[0:3], self.board[3:6], self.board[:6],self.board[0:7:3], \
            self.board[1:8:3], self.board[1::3], self.board[2::3],  self.board[0::4], self.board[2:7:2]]
